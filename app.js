...
const S = {
  ...
  currentFrontHash: null,
  currentBackHash: null,
  detailRecord: null,
  detailPhotos: [],
  authBusy: false,
  currentEventId: null,
  currentEventSakeId: null,
  currentSakeId: null,
  currentSaveRequestId: null,
  currentEventPhotoRequestId: null,
...
window.addEventListener("online", async () => {
  ...
  if (S.user) {
    ...
    if ($("recoveryNotice")?.classList.contains("hidden"))
      maybeShowDraftRecoveryNotice();
  }
});
...
window.addEventListener("unhandledrejection", (e) => {
  ...
  const text =
    e.reason?.message || String(e.reason || "処理エラーが発生しました");
  const el = $("recordMsg") || $("editMsg");
  if (el) {
    el.innerHTML =
      '<div class="msg err">処理エラー：' + escapeHtml(text) + "</div>";
  }
});
...
function show(id) {
  ...
  [
    "setupView",
    "authView",
    ...Array.from(document.querySelectorAll(".page")).map((x) => x.id),
  ].forEach((x) => $(x)?.classList.add("hidden"));
  $(id).classList.remove("hidden");
  const focusedRecordViews = new Set([
    "captureView",
    "frontConfirmView",
    "backCaptureView",
    "backConfirmView",
  ...
  if (id === "recordView") {
    if ($("drankDate") && !$("drankDate").value)
      $("drankDate").value = todayDateValue();
    syncRecordSummary();
    syncRatingChoices();
  }
...
function saveSession(d) {
  ...
  if (S.refreshToken)
    localStorage.setItem("sakelog_refresh_token", S.refreshToken);
...
async function startLineLink() {
  ...
  try {
    ...
    if (!r.ok || !d?.authorize_url)
      throw new Error(d?.error || "LINE連携を開始できませんでした");
  ...
  try {
    ...
    for (const [id, key] of map) {
      ...
      if (el)
        el.classList.toggle(
          "hidden",
          key === "custom:line" ? false : ext[key] !== true,
        );
    }
  ...
  if (lineTokenHash) {
    try {
      ...
      if (!r.ok || !d?.access_token || !d?.user)
        throw new Error(
          d?.msg || d?.message || "LINEログインの完了に失敗しました",
        );
      saveSession(d);
      history.replaceState(null, "", location.pathname + location.search);
      return true;
...
async function refreshSession() {
  ...
  if (!r.ok || !d.access_token)
    throw new Error(d.message || d.msg || "ログインの更新に失敗しました。");
  saveSession(d);
  return true;
}
async function authFetch(url, options = {}) {
  const make = async () => {
    try {
      ...
      if (
        ...
        /fetch|network|offline/i.test(String(e?.message || e))
      ) {
        throw new Error(
          "通信できません。電波状況を確認してもう一度お試しください。",
        );
      }
      throw e;
    }
  ...
  if (r.status === 401) {
    const t = await r
      ...
      .catch(() => "");
    if (
      /jwt expired|expired|invalid jwt|jwt issued at future|issued at future|not yet valid/i.test(
        t,
      )
    ) {
      try {
      ...
      r = await make();
      if (r.status === 401) {
        const retryText = await r
          .clone()
          .text()
          .catch(() => "");
        if (
          /jwt issued at future|issued at future|not yet valid/i.test(retryText)
        ) {
          clearSession();
          show("authView");
          throw new Error(
            "端末の日時がサーバーとずれています。スマホの「日付と時刻」を自動設定にして、もう一度ログインしてください。",
          );
        }
      }
    }
...
async function testConnection() {
  ...
  if (!key.startsWith("sb_publishable_")) {
    msg($("setupMsg"), "Publishable keyを確認してください。", "err");
    return false;
  }
  try {
    const r = await fetch(BASE + "/auth/v1/settings", {
    ...
    msg($("setupMsg"), "✓ Supabaseに接続できました。HTTP " + r.status, "ok");
    return true;
  } catch (e) {
    msg($("setupMsg"), "接続失敗: " + e.message, "err");
    return false;
  }
}
$("key").value = cfgKey();
...
$("saveSetup").onclick = async () => {
  if (await testConnection()) {
    localStorage.setItem("sakelog_pubkey", $("key").value.trim());
    show("authView");
  }
};
...
$("authBtn").onclick = async () => {
  ...
  if (!email) {
    msg($("authMsg"), "メールアドレスを入力してください。", "err");
    return;
  }
  if (S.signup && password.length < 8) {
    msg($("authMsg"), "新規登録のパスワードは8文字以上にしてください。", "err");
    return;
  }
  if (!S.signup && password.length < 6) {
    msg($("authMsg"), "パスワードを入力してください。", "err");
    return;
  }
  S.authBusy = true;
  ...
  try {
    ...
    if (S.signup && !d.access_token) {
      msg(
        $("authMsg"),
        "✓ 登録を受け付けました。確認メールが届く場合はメール認証後にログインしてください。",
        "ok",
      );
      return;
    }
...
function clearSession() {
  [
    "sakelog_token",
    "sakelog_refresh_token",
    "sakelog_user",
    "sakelog_access_token",
...
async function verifyCurrentUser() {
  ...
  const r = await fetch(BASE + "/auth/v1/user", {
    headers: { apikey: cfgKey(), Authorization: "Bearer " + S.token },
  });
...
async function restore() {
  ...
  if (await verifyCurrentUser()) {
    ...
    if (localStorage.getItem("sakelog_line_linked_notice") === "1") {
      ...
      alert(
        "LINEをこの和酒ログアカウントに連携しました。次回からLINEでも同じ記録に入れます。",
      );
    }
  } else {
    clearSession();
...
function replaceFrontPhotoWithoutReset(file) {
  ...
  msg(
    $("recordMsg"),
    "表ラベルを変更しました。入力済みの内容はそのまま残しています。",
    "info",
  );
}
function replaceBackPhotoWithoutReset(file) {
  ...
  msg(
    $("recordMsg"),
    "裏ラベルを変更しました。入力済みの内容はそのまま残しています。",
    "info",
  );
}
...
function resetRecord(openView = true) {
  ...
  S.currentEventSakeId = null;
  S.currentSakeId = null;
  S.currentSaveRequestId = null;
  [
    "brand",
    "product",
  ...
  ["preview", "backPreview", "foodPreview", "memoryPreview"].forEach((id) =>
    $(id)?.classList.add("hidden"),
  );
  $("foodCameraBtn")?.classList.remove("has-photo");
  $("memoryCameraBtn")?.classList.remove("has-photo");
  [
    "cameraInput",
    "galleryInput",
    "backCameraInput",
  ...
  msg($("analysisMsg"), "");
  if ($("recordValidationMsg")) $("recordValidationMsg").innerHTML = "";
  [
    "polishing",
    "alcohol",
    "volume",
    ...
    "product",
    "brewery",
  ].forEach((id) => $(id)?.classList.remove("field-error", "field-warning"));
  $("rating").value = 4;
...
function recordDateISOString() {
  const value = $("drankDate")?.value;
...
function syncRatingChoices() {
  const value = Number($("rating")?.value || 4);
...
function syncRecordSummary() {
  const title = [$("brand")?.value, $("product")?.value]
    .map((value) => String(value || "").trim())
    .filter(Boolean)
    .join(" ");
  if ($("recordSakeName"))
    $("recordSakeName").textContent = title || "銘柄を入力してください";
  if ($("recordBreweryName"))
    $("recordBreweryName").textContent =
      $("brewery")?.value?.trim() || "蔵元は未入力です";
}
$("rating").oninput = () => {
...
$("recordEditSakeBtn").onclick = () => {
  $("recordSakeDetails").open = true;
  $("brand")?.focus();
};
["brand", "product", "brewery"].forEach((id) =>
  $(id)?.addEventListener("input", syncRecordSummary),
);
...
function attachFoodPhoto(file) {
  ...
  $("foodCameraBtn")?.classList.add("has-photo");
}
...
function renderSources(sources) {
  ...
  return `<div style="margin-top:12px"><b>Web確認元</b>${sources
    .map((s) => {
...
function renderCandidates(items) {
  // 同一商品の重複候補をまとめる
  const normalizeCandidateText = (v) =>
    String(v || "")
      .toLowerCase()
  ...
  if (!items?.length) {
    ...
    if (hasLabelIdentity) {
      $("candidateBox").innerHTML =
        '<div class="msg info"><b>📷 ラベル読取候補</b><br>' +
        escapeHtml(
          [f.brand, f.product].filter(Boolean).join(" ") || "商品名を一部読取",
        ) +
        (f.brewery
          ? '<br><span class="small">' + escapeHtml(f.brewery) + "</span>"
          : "") +
        '<br><span class="small">Web確認はできませんでしたが、写真から読めた情報で記録できます。</span></div>';
    } else {
      $("candidateBox").innerHTML =
        '<div class="msg err"><b>ラベルから商品名を十分に読み取れませんでした。</b><br>裏ラベルを正面から大きく撮るか、別角度の写真を試してください。</div>';
    }
    return;
  }
  $("candidateBox").innerHTML =
    ...
    uniqueItems
      .map((c, i) => {
        const pct = Math.round(
          Math.max(0, Math.min(1, Number(c.confidence) || 0)) * 100,
        );
        ...
        const checks = [
          c.evidence?.front_label
            ? `表: ${escapeHtml(c.evidence.front_label)}`
            : "",
          c.evidence?.back_label
            ? `裏: ${escapeHtml(c.evidence.back_label)}`
            : "",
          c.evidence?.web_match
            ? `Web: ${escapeHtml(c.evidence.web_match)}`
            : "",
        ]
          .filter(Boolean)
          .join("<br>");
...
const BAD_MASTER_WORDS = new Set([
  "確認中",
  "未確認",
  "不明",
  "不詳",
...
function isUsableMasterBrand(v) {
  ...
  return (
    x.length > 0 &&
    !BAD_MASTER_WORDS.has(x) &&
    !/^(確認|未確認|不明|不詳|unknown|候補|解析中|未判定)/i.test(x)
  );
}
...
async function learnToSakeMaster(values) {
  ...
  if (!isUsableMasterBrand(brand))
    return { skipped: true, reason: "invalid_brand" };
  try {
...
function normJP(s) {
  return String(s || "")
    .normalize("NFKC")
    .toLowerCase()
    .replace(/株式会社|有限会社|合資会社|合同会社|酒造/g, "")
    .replace(/[\s　・･\-ー_（）()「」『』【】]/g, "");
}
function grams(s) {
  const x = normJP(s),
  ...
  for (let i = 0; i < x.length - 1; i++) a.push(x.slice(i, i + 2));
  return a;
}
function simText(a, b) {
  ...
  if (x.includes(y) || y.includes(x))
    return (
      0.6 + (0.35 * Math.min(x.length, y.length)) / Math.max(x.length, y.length)
    );
  const A = grams(x),
    B = grams(y),
    used = new Set();
  let hit = 0;
  for (const q of A) {
    const j = B.findIndex((v, i) => v === q && !used.has(i));
    if (j >= 0) {
      used.add(j);
      hit++;
    }
...
async function loadSakenowaMaster() {
  ...
  try {
    const r = await fetch("/.netlify/functions/sakenowa-master"),
      d = await r.json();
    if (!r.ok) throw 0;
    return (SAKENOWA_MASTER = d);
  } catch {
    return null;
  }
}
  ...
  try {
    ...
    for (const m of rows || []) {
      ...
      let score =
        ...
        breweryScore * 0.16 +
        prefScore * 0.04;
      if (brandScore >= 0.99) score += 0.08;
      if (productScore >= 0.99) score += 0.08;
      if (breweryScore >= 0.9) score += 0.04;
...
async function matchSakenowaFacts(f) {
  const m = await loadSakenowaMaster();
  if (!m) return [];
  const breweries = new Map(m.breweries.map((x) => [x.id, x])),
    areas = new Map(m.areas.map((x) => [x.id, x]));
  const qb = f?.brand || f?.product || "",
    qbr = f?.brewery || "",
    qp = f?.prefecture || "",
    out = [];
  for (const b of m.brands) {
    const br = breweries.get(b.breweryId) || {},
      ar = areas.get(br.areaId) || {};
    const bs = simText(qb, b.name),
      brs = qbr ? simText(qbr, br.name) : 0,
      ps = qp ? simText(qp, ar.name) : 0;
    let score = bs * 0.76 + brs * 0.18 + ps * 0.06;
    if (bs >= 0.99) score += 0.08;
    if (brs >= 0.9) score += 0.05;
    if (score >= 0.42)
      out.push({
        brand: b.name,
        brewery: br.name || "",
...
async function loadUpcomingEventsForSake(sakeId) {
  if (!sakeId) return [];
  try {
    const rows = await rpc("upcoming_events_for_sake", {
      p_sake_id: sakeId,
      p_limit: 5,
    });
...
function renderUpcomingEventsForSake(items) {
  ...
  box.innerHTML =
    ...
    items
      .map((e) => {
        const place = [e.prefecture, e.city, e.venue_name]
          .filter(Boolean)
          .join(" ");
...
function renderSakenowaMatches(items) {
  const old = document.getElementById("sakenowaMatches");
  if (old) old.remove();
  if (!items?.length) return;
  const box = document.createElement("div");
  box.id = "sakenowaMatches";
  box.className = "sakenowa-box";
  box.innerHTML =
    "<b>🍶 さけのわ銘柄DB照合</b>" +
    items
      ...
      .map(
        (x) =>
          `<div class="sakenowa-candidate"><b>${escapeHtml(x.brand)}</b>${x.brewery ? " ／ " + escapeHtml(x.brewery) : ""}${x.prefecture ? "（" + escapeHtml(x.prefecture) + "）" : ""} <span class="small">銘柄一致度 ${Math.round(x.score * 100)}%</span></div>`,
      )
      .join("") +
    '<div class="small"><a href="https://sakenowa.com" target="_blank" rel="noopener">さけのわデータ</a>を利用しています。</div>';
  $("analysisMsg").insertAdjacentElement("afterend", box);
}
...
async function lookupSharedDictionary(frontHash, backHash) {
  try {
    ...
    const ok = both
      ? best.sim.score >= 0.9 && best.sim.front >= 0.84 && best.sim.back >= 0.84
      : best.sim.score >= 0.95;
    return ok ? best : null;
  } catch (e) {
...
async function saveToSharedDictionary() {
  ...
  try {
    ...
    if (obj.front_hash)
      filters.push("front_hash=eq." + encodeURIComponent(obj.front_hash));
    if (obj.back_hash)
      filters.push("back_hash=eq." + encodeURIComponent(obj.back_hash));
    const rows = await select(
      DICT_TABLE,
      filters.join("&") + "&order=created_at.desc&limit=5",
    ).catch(() => []);
    let same = null;
    for (const row of rows) {
...
async function buildImageCacheKey(frontData, backData) {
  return (
    AI_CACHE_PREFIX +
    (await sha256Text(String(frontData || "") + "|" + String(backData || "")))
  );
}
...
function confirmedKey(aiKey) {
  return (
    CONFIRMED_CACHE_PREFIX + String(aiKey || "").replace(AI_CACHE_PREFIX, "")
  );
}
...
async function findSupabasePastRecord(facts) {
  ...
  try {
    ...
    if (facts.product)
      q += "&product_name=eq." + encodeURIComponent(facts.product);
...
async function uploadEventPhoto(file, eventId, requestId) {
  if (!file || !eventId || !requestId)
    throw new Error("写真またはイベントが選択されていません");
  ...
  if (!r.ok) {
    ...
    throw new Error(
      "イベント写真保存エラー HTTP " + r.status + (detail ? " " + detail : ""),
    );
  }
  return path;
}
...
async function addEventPhoto(file) {
  ...
  if (
    !requireOnline(
      "現在オフラインです。イベント写真は通信が戻ってから追加してください。",
    )
  )
    return;
  ...
  try {
    ...
    if (pending?.storagePath) {
      try {
        await deleteStorageObjectFromBucket(
          "event-photos",
          pending.storagePath,
        );
      } catch {}
...
async function loadProfileEventStats() {
  ...
  try {
    ...
    box.innerHTML =
      '<div class="small">イベント記録を読み込めませんでした。</div>';
  }
}
...
async function loadProfileEventDiscoveries() {
  ...
  try {
    ...
    const latest = Array.isArray(s?.latest_discoveries)
      ? s.latest_discoveries
      : [];
    box.innerHTML = `
    ...
    box.innerHTML =
      '<div class="small">イベントでの出会いを読み込めませんでした。</div>';
  }
}
  ...
  try {
    ...
    box.innerHTML = `
      ...
      `,
              ...
              .join("")
          : '<div class="small" style="margin-top:7px">イベントで飲んだ酒をLOGすると、ここにベスト3が育っていきます。</div>'
      }
    `;
  } catch (e) {
    console.warn("profile event year review failed", e);
    box.innerHTML =
      '<div class="small">今年の振り返りを読み込めませんでした。</div>';
  }
}
...
async function requestMyEventPhotoDeletion(photoId) {
  if (
    !requireOnline(
      "現在オフラインです。削除依頼は通信が戻ってから送信してください。",
    )
  )
    return;
...
function eventPhotoStatusLabel(p) {
  ...
  if (
    ...
    return "一般公開中";
  if (p.moderation_status === "approved" && p.audience === "event_members")
    return "参加者内のみ";
...
async function loadProfileEventPhotos() {
  ...
  try {
    ...
    box.innerHTML =
      '<div class="small">イベント写真を読み込めませんでした。</div>';
  }
}
...
async function loadProfileEventHistory() {
  ...
  try {
    const parts = await select(
      ...
      "select=event_id,status,created_at,updated_at&user_id=eq." +
        encodeURIComponent(S.user.id) +
    ...
    if (!parts.length) {
      box.innerHTML =
        '<div class="small">イベントの予定・参加履歴はまだありません。</div>';
      return;
    }
    ...
    for (const e of rows) {
      ...
      const isPast =
        ...
        ["checked_in", "attended"].includes(e.participation_status);
    ...
    const renderEventRow = (e, isPast) => {
      const place = [e.prefecture, e.city, e.venue_name]
        ...
        .join(" ");
      const status = isPast
        ? e.participation_status === "attended"
          ? "参加済み"
          : e.participation_status === "checked_in"
            ? "参加記録あり"
            : "過去の予定"
        : "参加予定";
      return `
    ...
    box.innerHTML =
      ...
      (upcoming.length
        ? upcoming.map((e) => renderEventRow(e, false)).join("")
        : '<div class="small" style="margin-top:6px">予定はありません。</div>') +
      '<div style="margin-top:18px"><b>行ったイベント</b></div>' +
      (past.length
        ? past.map((e) => renderEventRow(e, true)).join("")
        : '<div class="small" style="margin-top:6px">参加履歴はまだありません。</div>');
  } catch (e) {
    console.warn("profile event history failed", e);
    box.innerHTML =
      '<div class="small">イベント履歴を読み込めませんでした。</div>';
  }
}
...
async function loadMyUpcomingEventList() {
  ...
  try {
    const parts = await select(
      "event_participations",
      "select=event_id,status&user_id=eq." +
        encodeURIComponent(S.user.id) +
    ...
    if (!rows.length) {
      box.innerHTML =
        '<div class="small">これから参加予定のイベントはありません。</div>';
      return;
    }
...
async function showEvents() {
  ...
  const rows = (await loadPublishedEvents()).filter(
    (e) => !e.ends_at || new Date(e.ends_at) >= new Date(),
  );
  if (!rows.length) {
    $("eventBody").innerHTML =
      "<h2>📅 日本酒イベント</h2><p class='small'>現在、公開中のイベント情報はありません。</p>";
    return;
  }
  $("eventBody").innerHTML =
    "<h2>📅 日本酒イベント</h2>" +
...
async function loadPreviousEventPublicPhotos(previousEventId, limit = 6) {
  ...
  try {
    const rows = await select(
      "event_photos",
      "select=id,storage_path,caption,created_at&event_id=eq." +
        encodeURIComponent(previousEventId) +
        "&audience=eq.public&is_public=eq.true&moderation_status=eq.approved" +
        "&face_review_status=in.(no_face,processed)&order=created_at.desc&limit=" +
  ...
  try {
    ...
    for (const row of Array.isArray(rows) ? rows : []) {
      ...
      if (row.front_photo_path) {
        photoUrl = await createSignedStorageUrl(
          "sake-photos",
...
function renderMyEventDrinkSummary(items) {
  if (!items?.length) return "";
  const avg =
    items.reduce((s, x) => s + Number(x.rating || 0), 0) / items.length;
...
async function loadPreviousEventMemory(eventId) {
  try {
    const rows = await rpc("previous_event_memory_summary", {
      p_event_id: eventId,
    });
...
function renderPreviousEventMemory(memory) {
  if (!memory) return "";
  const place = [
    memory.previous_prefecture,
    memory.previous_city,
    memory.previous_venue_name,
  ]
    .filter(Boolean)
    .join(" ");
  return `
...
async function loadMyEventParticipation(eventId) {
  try {
    const rows = await select(
      "event_participations",
      "select=id,status,created_at,updated_at&event_id=eq." +
        encodeURIComponent(eventId) +
        "&user_id=eq." +
        encodeURIComponent(S.user.id) +
        "&limit=1",
...
function unlockEventAction(key) {
  eventActionLocks.delete(key);
}

function setEventActionButtonsDisabled(eventId, disabled) {
  document
    .querySelectorAll(
      '[data-event-action="' + CSS.escape(String(eventId)) + '"]',
    )
    .forEach((btn) => {
      btn.disabled = disabled;
      btn.style.opacity = disabled ? ".55" : "";
...
async function checkInCurrentEvent(eventId) {
  if (
    !requireOnline(
      "現在オフラインです。参加記録は通信が戻ってから残してください。",
    )
  )
    return;
...
function canShowEventCheckIn(e) {
  try {
    ...
    const end =
      new Date(e.ends_at || e.starts_at).getTime() + 12 * 60 * 60 * 1000;
...
async function joinCurrentEvent(eventId) {
  if (
    !requireOnline(
      "現在オフラインです。参加予定への追加は通信が戻ってから行ってください。",
    )
  )
    return;
  ...
  try {
    ...
    else if (/does not accept|no_application|invite/i.test(t))
      alert(
        "このイベントはアプリ内から参加申込できません。公式案内をご確認ください。",
      );
...
async function leaveCurrentEvent(eventId) {
  if (
    !requireOnline(
      "現在オフラインです。参加予定の変更は通信が戻ってから行ってください。",
    )
  )
    return;
...
async function openEventDetail(eventId) {
  ...
  try {
    ...
    const sakes = await select(
      "event_sakes",
      "select=id,sake_id,brand_name,product_name,brewery_name,classification,serving_note,beverage_category,master_eligible,display_order&event_id=eq." +
        encodeURIComponent(eventId) +
    ...
    $("eventBody").innerHTML = `
      ...
      `,
              ...
              .join("")
          : "<p class='small'>出品酒情報は準備中です。</p>"
      }
    `;
  } catch (e) {
    $("eventBody").innerHTML =
      "<p>取得エラー：" + escapeHtml(e.message) + "</p>";
  }
}
...
async function logEventSake(eventId, eventSakeId, sakeId) {
  try {
    const rows = await select(
      "event_sakes",
      "select=brand_name,product_name,brewery_name,classification,sake_id&event_id=eq." +
        encodeURIComponent(eventId) +
        "&id=eq." +
        encodeURIComponent(eventSakeId) +
        "&limit=1",
    );
    const s = rows?.[0];
    ...
    msg(
      $("recordMsg"),
      "📅 イベントの出品酒から記録を開始しました。写真を追加しても、そのままLOGしてもOKです。",
      "info",
    );
    saveRecordDraft();
  } catch (e) {
...
$("analyzeBtn").onclick = async () => {
  ...
  msg(
    $("analysisMsg"),
    "📚 共有辞書 → 確定済み記録 → AIキャッシュ → AI解析の順で探します…",
    "info",
  );
  try {
    const frontData = S.photo ? await fileToDataURL(S.photo, "front") : null;
    const backData = S.backPhoto
      ? await fileToDataURL(S.backPhoto, "back")
      : null;
    if (!frontData && !backData)
      throw new Error("ラベル写真を選択してください");
    ...
    let swDict = [];
    const dict = await lookupSharedDictionary(
      S.currentFrontHash,
      S.currentBackHash,
    );
    if (dict) {
      ...
      S.recognition = {
        ...
        candidates: [
          {
            brand: dict.row.brand_name || "",
            product: dict.row.product_name || "",
            brewery: dict.row.brewery_name || "",
            prefecture: dict.row.prefecture || "",
            classification: dict.row.classification || "",
            confidence: dict.sim.score,
            web_verified: false,
    ...
    if (confirmed) {
      ...
      msg(
        $("analysisMsg"),
        "✅ 同じ写真の「確定済み記録」を再利用しました。AIは使っていません。",
        "ok",
      );
    }
    ...
    if (dict) {
      msg(
        $("analysisMsg"),
        `📚 酒ログ共有辞書から候補を見つけました（画像一致度 ${Math.round(dict.sim.score * 100)}%）。AIは使っていません。${swDict[0] && swDict[0].score >= 0.72 ? " さけのわ銘柄DBでも確認できました。" : ""} 現物ラベルを確認して保存してください。`,
        "ok",
      );
      return;
    }
    ...
    if (d) {
      ...
      msg(
        $("analysisMsg"),
        "共有辞書に未登録の新しい日本酒です。AIでラベル読取＋Web照合を行っています…",
        "info",
      );
      const body = { front: frontData, back: backData };
      const r = await authFetch("/.netlify/functions/recognize-sake", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    d = {
      ...d,
      candidates: mergeRecognitionCandidates(d.candidates || [], masterMatches),
    };
    S.recognition = d;
    ...
    if (top) {
      ...
      $("rice").value = f.ingredients || "";
      if ($("riceVariety"))
        $("riceVariety").value = f.rice_variety || f.rice || "";
      $("polishing").value = f.polishing_ratio || "";
      $("alcohol").value = f.alcohol || "";
      $("volume").value = f.volume || "";
    }
    ...
    const sw = await matchSakenowaFacts({
      brand: $("brand").value.trim() || f.brand || "",
      product: $("product").value.trim() || f.product || "",
      brewery: $("brewery").value.trim() || f.brewery || "",
      prefecture: $("prefecture").value.trim() || f.prefecture || "",
    });
    renderSakenowaMatches(sw);
    // さけのわは銘柄・蔵元・地域の正規化だけ。個体スペックは上書きしない。
    if (sw[0] && sw[0].score >= 0.88) {
    ...
    if (d.cache_hit) {
      msg(
        $("analysisMsg"),
        "♻️ AI解析済みキャッシュを再利用しました。APIは使っていません。",
        "ok",
      );
    } else {
      msg(
        ...
        top?.web_verified
          ...
          : "📷 写真から読めた情報を自動入力しました。保存すると酒ログ共有辞書にも学習されます。",
        "ok",
      );
    }
  } catch (e) {
    ...
    if (/本日のAI認識上限|短時間のAI認識回数|rate limit|429|TPM/i.test(text)) {
      msg(
        $("analysisMsg"),
        "AI認識の利用上限に達しました。写真はそのまま残っています。時間をおいて再度試すか、このまま手入力・「確認中」で記録できます。",
        "err",
      );
    } else if (/通信できません|failed to fetch|network|offline/i.test(text)) {
      msg(
        $("analysisMsg"),
        "通信できませんでした。写真と入力内容は残っています。通信が戻ってからもう一度「AIで銘柄候補を探す」を押してください。",
        "err",
      );
    } else if (/時間内に完了|timeout|timed out/i.test(text)) {
      msg(
        $("analysisMsg"),
        "解析に時間がかかっています。写真は残っているので、少し時間をおいてもう一度試せます。このまま記録することもできます。",
        "err",
      );
    } else {
      msg(
        $("analysisMsg"),
        "銘柄候補を取得できませんでした。写真と入力内容は残っています。このまま手入力、または「確認中」で記録できます。",
        "err",
      );
    }
  } finally {
...
async function preparePhotoForUpload(file) {
  ...
  try {
    if (
      ...
      /^(image\/jpeg|image\/webp)$/i.test(file.type || "")
    ) {
      return file;
    }
    ...
    try {
      ...
      const blob = await new Promise((resolve, reject) =>
        c.toBlob(
          (b) => (b ? resolve(b) : reject(new Error("画像圧縮に失敗しました"))),
          "image/jpeg",
          0.88,
        ),
      );
...
async function uploadPhoto(file) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      const detail = await r.text().catch(() => "");
      lastError = new Error(
        "写真保存エラー HTTP " + r.status + (detail ? " " + detail : ""),
      );
...
function restoreRecordDraftToForm() {
  ...
  show("recordView");
  const photoNote =
    d.hadFrontPhoto || d.hadBackPhoto || d.hadFoodPhoto || d.hadMemoryPhoto
      ? " 写真は端末の安全上、自動復元できないため必要な写真だけ選び直してください。"
      : "";
  msg(
    $("recordMsg"),
    "前回入力途中だった内容を復元しました。" + photoNote,
    "info",
  );
  return true;
}
...
function clearLocalOperation(key) {
  writeLocalOperation(key, null);
}
...
function showRecoveryNotice(html) {
  ...
  box.innerHTML =
    '<div class="card" style="border-color:#d8c9a9;background:#fffaf1">' +
    html +
    "</div>";
      ...
      try {
        const rows = await select(
          ...
          "select=id,brand_name,product_name,brewery_name,created_at&client_request_id=eq." +
            encodeURIComponent(pendingSave.requestId) +
            "&limit=1",
        );
        const rec = rows?.[0];
        if (rec) {
          const photos = await select(
            "sake_photos",
            "select=photo_type&drinking_record_id=eq." +
              encodeURIComponent(rec.id),
          ).catch(() => []);
          const have = new Set((photos || []).map((x) => x.photo_type));
          const expected = Array.isArray(pendingSave.expectedPhotoTypes)
            ? pendingSave.expectedPhotoTypes
            : [];
          ...
          if (missing.length) {
            notices.push(
              "<b>前回のLOGは途中まで保存されています。</b>" +
                ...
                '<button class="btn outline" style="margin-top:9px" onclick="openRecordDetail(\'' +
                escapeHtml(rec.id) +
                "')\">前回のLOGを確認</button>",
            );
          } else {
  ...
  if (pendingPhoto) {
    if (pendingPhoto.userId !== S.user.id) {
      ...
      try {
        const rows = await select(
          "event_photos",
          "select=id,event_id&client_request_id=eq." +
            encodeURIComponent(pendingPhoto.requestId) +
            "&limit=1",
        );
        if (rows?.length) {
          notices.push(
...
async function insert(table, obj) {
  const r = await authFetch(BASE + "/rest/v1/" + table, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Prefer: "return=representation",
...
async function select(table, query = "") {
  const r = await authFetch(BASE + "/rest/v1/" + table + "?" + query, {
...
function writePendingRollbacks(items) {
  try {
    if (items.length)
      localStorage.setItem(PENDING_ROLLBACK_KEY, JSON.stringify(items));
...
function queuePendingRollback(recordId, storagePaths = []) {
  ...
  items.push({
    recordId,
    storagePaths: [...new Set(storagePaths.filter(Boolean))],
    queuedAt: Date.now(),
  });
...
function readPendingStorageDeletes() {
  try {
    const v = JSON.parse(
      localStorage.getItem(PENDING_STORAGE_DELETE_KEY) || "[]",
    );
    return Array.isArray(v) ? v : [];
  } catch {
    return [];
...
function writePendingStorageDeletes(items) {
  try {
    ...
    if (clean.length)
      localStorage.setItem(PENDING_STORAGE_DELETE_KEY, JSON.stringify(clean));
  ...
  try {
    ...
    for (const path of paths) {
      try {
        ...
        queuePendingStorageDelete(path);
      }
    }
    return true;
  } catch (e) {
...
function valuesClearlyDifferent(a, b, threshold = 0.45) {
  const aa = normalizedCompareText(a),
    bb = normalizedCompareText(b);
  if (!aa || !bb) return false;
  if (aa === bb || aa.includes(bb) || bb.includes(aa)) return false;
  try {
    return simText(a, b) < threshold;
  } catch {
    return aa !== bb;
  }
}
function recognitionReference() {
  ...
  const f = S.recognition.label_facts || {};
  const c =
    S.selectedRecognitionCandidate || (S.recognition.candidates || [])[0] || {};
  return {
...
function recognitionDiscrepancies() {
  ...
  if (valuesClearlyDifferent(current.brand, ref.brand, 0.4)) {
    out.push({
      field: "brand",
      label: "銘柄",
      read: ref.brand,
  ...
  if (valuesClearlyDifferent(current.product, ref.product, 0.38)) {
    out.push({
      field: "product",
      label: "商品名",
      read: ref.product,
  ...
  if (valuesClearlyDifferent(current.brewery, ref.brewery, 0.42)) {
    out.push({
      field: "brewery",
      label: "蔵元",
      read: ref.brewery,
      input: current.brewery,
    });
  }

  const pNow = firstNumber(current.polishing_ratio),
    pRef = firstNumber(ref.polishing_ratio);
  if (pNow !== null && pRef !== null && Math.abs(pNow - pRef) >= 5) {
    out.push({
      field: "polishing",
      label: "精米歩合",
      read: ref.polishing_ratio,
      input: current.polishing_ratio,
    });
  }
  const aNow = firstNumber(current.alcohol),
    aRef = firstNumber(ref.alcohol);
  if (aNow !== null && aRef !== null && Math.abs(aNow - aRef) >= 2) {
    out.push({
      field: "alcohol",
      label: "アルコール度数",
      read: ref.alcohol,
      input: current.alcohol,
    });
  }
  const vNow = volumeMl(current.volume),
    vRef = volumeMl(ref.volume);
...
function validateRecordForm({ render = true } = {}) {
  ...
  if (polishing !== null) {
    if (polishing <= 0 || polishing > 100) {
      errors.push("精米歩合は1〜100%の範囲で確認してください。");
      fields.polishing = "error";
    } else if (polishing < 20) {
      warnings.push(
        "精米歩合が20%未満です。ラベルの数字をもう一度確認してください。",
      );
      fields.polishing = "warning";
    }
  }
  ...
  if (alcohol !== null) {
    if (alcohol < 0 || alcohol > 100) {
      errors.push("アルコール度数は0〜100%の範囲で確認してください。");
      fields.alcohol = "error";
    } else if (alcohol < 5 || alcohol > 30) {
      warnings.push(
        "アルコール度数が一般的な日本酒の範囲から外れています。ラベル表記を確認してください。",
      );
      fields.alcohol = "warning";
    }
  }
  ...
  if (ml !== null) {
    if (ml <= 0) {
      errors.push("容量は0より大きい値を入力してください。");
      fields.volume = "error";
    } else if (ml < 100 || ml > 10000) {
      warnings.push("容量の値が珍しいため、ml / L の単位を確認してください。");
      fields.volume = "warning";
    }
  }
  ...
  if (priceText) {
    ...
    if (!Number.isFinite(price) || price < 0) {
      errors.push("価格は0円以上の数字で入力してください。");
      fields.price = "error";
    } else if (price > 1000000) {
      warnings.push("価格が100万円を超えています。桁数を確認してください。");
      fields.price = "warning";
    }
  }
  ...
  if (pref && !JAPAN_PREFECTURES.includes(pref)) {
    warnings.push(
      "都道府県名が標準表記と異なります。例：奈良県、京都府、東京都。",
    );
  ...
  if (brand && product && brand === product) {
    warnings.push(
      "銘柄と商品名が同じ文字になっています。分け方を確認してください。",
    );
    fields.brand = fields.product = "warning";
  }
  if (
    brewery &&
    !/(酒造|酒蔵|醸造|酒店|株式会社|有限会社|合名会社|合同会社|本家|本舗)/.test(
      brewery,
    ) &&
    brewery.length <= 3
  ) {
  ...
  if (render) {
    ...
    if (box) {
      ...
      if (errors.length)
        parts.push(
          '<div class="msg err"><b>入力を確認してください</b><br>' +
            errors.map(escapeHtml).join("<br>") +
            "</div>",
        );
      if (warnings.length)
        parts.push(
          '<div class="msg info"><b>念のため確認</b><br>' +
...
function recordInputDateJP() {
  const value = $("drankDate")?.value;
  if (!value) return "今日";
  const [year, month, day] = value.split("-");
  return `${Number(year)}年${Number(month)}月${Number(day)}日`;
}
function renderRecordConfirmation(warnings = []) {
  const title = [$("brand").value, $("product").value]
  ...
  const rows = [
    ["日付", recordInputDateJP()],
    ["誰と", $("companion").value.trim()],
    ["どこで", $("restaurant").value.trim()],
    ["ひとこと", $("comment").value.trim()],
  ].filter(([, value]) => value);
  const memories = [$("foodPreview"), $("memoryPreview")]
    .filter((image) => image?.src && !image.classList.contains("hidden"))
    .map((image) => `<img src="${escapeHtml(image.src)}" alt="この日の思い出" />`)
    .join("");
  $("recordConfirmBody").innerHTML = `
    ${previewSrc ? `<div class="record-confirm-hero"><img src="${escapeHtml(previewSrc)}" alt="撮影した日本酒" /></div>` : ""}
    <div class="record-confirm-title"><strong>${escapeHtml(title)}</strong><span>${escapeHtml($("brewery").value.trim() || "蔵元未入力")}</span></div>
    <div class="record-confirm-rating" aria-label="評価 ${escapeHtml($("rating").value)}点"><strong>${escapeHtml($("rating").value)}</strong><span>/ 5</span></div>
    <dl class="record-confirm-list">${rows.map(([label, value]) => `<div class="record-confirm-row"><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value)}</dd></div>`).join("")}</dl>
    ${memories ? `<div class="record-confirm-memories">${memories}</div>` : ""}
    ${warnings.length ? `<div class="msg info"><b>念のため確認</b><br>${warnings.map(escapeHtml).join("<br>")}</div>` : ""}
  `;
}
$("saveRecordBtn").onclick = () => {
  ...
  if (!$("brand").value.trim() && !(S.photo || S.backPhoto)) {
  ...
  saveRecordDraft();
  renderRecordConfirmation(validation.warnings);
  msg($("recordConfirmMsg"), "");
  show("recordConfirmView");
};
...
$("commitRecordBtn").onclick = async () => {
  ...
  if (
    !requireOnline(
      "現在オフラインです。入力内容は消えません。通信が戻ってから保存してください。",
    )
  )
    return;
  const validation = validateRecordForm({ render: true });
  if (validation.errors.length) {
    msg(
      $("recordConfirmMsg"),
      "入力内容に確認が必要な項目があります。赤い欄を確認してください。",
      "err",
    );
    return;
  }
  ...
  if (!brand) {
    if (S.photo || S.backPhoto) {
      ...
      msg(
        $("recordConfirmMsg"),
        "銘柄を特定できなかったため「確認中」として保存します。あとから編集できます。",
        "info",
      );
    } else {
      S.savingRecord = false;
      msg($("recordConfirmMsg"), "銘柄を入力してください", "err");
      return;
    }
  }
  ...
  try {
    ...
    if (S.recognition) {
      const top =
        S.selectedRecognitionCandidate ||
        (S.recognition.candidates || [])[0] ||
        {};
      const recognitionDiffFields = recognitionDiscrepancies().map(
        (x) => x.field,
      );
      ...
      if (
        top.brand &&
        (brand !== top.brand ||
          ($("product").value.trim() || "") !== (top.product || ""))
      ) {
...
function showRecordComplete(rec) {
  ...
  const title =
    [rec.brand_name, rec.product_name].filter(Boolean).join(" ") ||
    "今日の一杯";
  ...
  $("completeBody").innerHTML = `
   ...
   <h2>この一杯を、<br>残しました。</h2>
   ${previewSrc ? `<div class="complete-photo"><img src="${escapeHtml(previewSrc)}" alt="記録した日本酒" /></div>` : ""}
   <div class="complete-sake">
     <strong>${escapeHtml(title)}</strong>
     ${brewery ? `<span>${escapeHtml(brewery)}</span>` : ""}
     ${rating ? `<div class="complete-rating" aria-label="評価 ${rating}点"><strong>${rating}</strong><small>/ 5</small></div>` : ""}
   </div>
   <p class="complete-copy">写真も、その日のことも。<br>いつでもMY LOGから振り返れます。</p>
   <div class="complete-actions"><button id="completeDetailBtn" class="btn primary">この記録を見る</button><button id="completeHomeBtn" class="confirm-text-button">ホームへ戻る</button></div>
 `;
  show("completeView");
...
function renderRows(rows, variant = "history") {
  ...
  if (variant === "history") {
    let currentMonth = "";
    return (
      '<div class="my-log-list">' +
      rows
        .map((r) => {
          const title = [r.brand_name, r.product_name]
            .filter(Boolean)
            .join(" ");
          const date = new Date(r.drank_at);
          const month = Number.isNaN(date.getTime())
            ? "日付未設定"
            : `${date.getFullYear()}年 ${date.getMonth() + 1}月`;
          const monthHeading = month !== currentMonth
            ? `<h3 class="my-log-month">${escapeHtml(month)}</h3>`
            : "";
          currentMonth = month;
          const meta = [r.companion_name, r.restaurant_name]
            .filter(Boolean)
            .join(" ・ ");
          ...
          const ratingText = Number.isInteger(rating)
            ? String(rating)
            : rating.toFixed(1);
          return `${monthHeading}<article class="my-log-entry" role="button" tabindex="0" aria-label="${escapeHtml(title || "名称未登録")}の記録を見る" data-record-id="${escapeHtml(r.id)}" onclick="openRecordDetail(\'${escapeHtml(r.id)}\')" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();openRecordDetail(\'${escapeHtml(r.id)}\')}">
            <div class="sake-thumb placeholder" data-thumb="${escapeHtml(r.id)}">写真なし</div>
            <div class="my-log-main">
              <div class="my-log-date">${escapeHtml(formatDateJP(r.drank_at))}</div>
              <div class="my-log-title">${escapeHtml(title || "名称未登録")}</div>
              <div class="my-log-brewery">${escapeHtml(r.brewery_name || "蔵元未登録")}</div>
              <div class="my-log-rating"><strong>${escapeHtml(ratingText)}</strong><span>/ 5</span></div>
              ${meta ? `<div class="my-log-meta">${escapeHtml(meta)}</div>` : ""}
              ${r.comment ? `<div class="my-log-comment">${escapeHtml(r.comment)}</div>` : ""}
            </div>
            <span class="my-log-arrow" aria-hidden="true">›</span>
          </article>`;
        })
        .join("") +
      "</div>"
    ...
    rows
      .map((r) => {
        ...
        const specs = [
          ...
          [
            "価格",
            r.price_yen ? Number(r.price_yen).toLocaleString() + "円" : "",
          ],
        ].filter((x) => x[1]);
     ...
     <div class="sake-main">
       ...
       <div class="sake-meta">${escapeHtml(formatDateJP(r.drank_at))}${r.companion_name ? " ・ " + escapeHtml(r.companion_name) : ""}${r.restaurant_name ? " ・ " + escapeHtml(r.restaurant_name) : ""}</div>
...
async function loadFrontPhotoMap(recordIds) {
  ...
  try {
    const q =
      "select=drinking_record_id,storage_path,photo_type&photo_type=eq.front&drinking_record_id=in.(" +
      clean.join(",") +
      ")";
    const rows = await select("sake_photos", q);
    const map = {};
    for (const p of rows || [])
      if (!map[p.drinking_record_id])
        map[p.drinking_record_id] = p.storage_path;
    return map;
  } catch {
    return {};
  }
...
async function storageObjectUrl(path) {
  ...
  try {
    const r = await authFetch(
      BASE + "/storage/v1/object/sake-photos/" + encodeURI(path),
      {},
    );
    if (!r.ok) return null;
  ...
  await Promise.all(
    rows.map(async (r) => {
      const el = document.querySelector(
        `#${containerId} [data-thumb="${CSS.escape(String(r.id))}"]`,
      );
      const path = map[r.id];
      if (!el || !path) return;
      ...
      img.src = url;
      img.alt =
        [r.brand_name, r.product_name].filter(Boolean).join(" ") || "日本酒";
      img.loading = "lazy";
      el.replaceWith(img);
    }),
  );
...
async function renderPhotoForPath(path, alt) {
  if (!path) return '<div class="detail-photo empty">写真なし</div>';
  const url = await storageObjectUrl(path);
  return url
    ? `<img class="detail-photo" src="${escapeHtml(url)}" alt="${escapeHtml(alt || "日本酒")}">`
    : '<div class="detail-photo empty">写真なし</div>';
}
async function openRecordDetail(recordId) {
  try {
    ...
    const frontHtml = await renderPhotoForPath(
      front?.storage_path,
      [r.brand_name, r.product_name].filter(Boolean).join(" "),
    );
    const backHtml = back
      ? await renderPhotoForPath(back.storage_path, "裏ラベル")
      : "";
    const foodHtml = food
    ...
    const ratingText = Number.isInteger(rating)
      ...
      : rating.toFixed(1);
    const facts = [
      ["日付", formatDateJP(r.drank_at)],
      ["誰と", r.companion_name],
      ["どこで", r.restaurant_name],
    ].filter(([, value]) => value);
    const specs = [
      ["都道府県", r.prefecture],
      ["種類", r.classification],
      ["原材料", r.rice],
      ["使用米・原料米", r.rice_variety],
      ["精米歩合", r.polishing_ratio],
      ["アルコール", r.alcohol],
    ].filter(([, value]) => value);
    $("detailBody").innerHTML = `
      <div class="memory-detail-front">${frontHtml}</div>
      <div class="memory-detail-title"><span>MY LOG</span><h2>${escapeHtml(title || "名称未登録")}</h2><p>${escapeHtml(r.brewery_name || "蔵元未登録")}</p></div>
      ${rating ? `<div class="memory-detail-rating" aria-label="評価 ${escapeHtml(ratingText)}点"><strong>${escapeHtml(ratingText)}</strong><span>/ 5</span></div>` : ""}
      <dl class="memory-detail-facts">${facts.map(([label, value]) => `<div class="memory-detail-fact"><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value)}</dd></div>`).join("")}</dl>
      ${r.comment ? `<div class="memory-detail-comment">${escapeHtml(r.comment)}</div>` : ""}
${
  foodHtml || memoryHtml
    ? `
<section class="memory-detail-section"><span>MEMORIES</span><h3>この日の景色</h3><div class="memory-detail-photos">${foodHtml}${memoryHtml}</div></section>
`
    : ""
}
      ${backHtml || specs.length ? `<details class="memory-sake-details"><summary>このお酒の詳しい情報</summary>${backHtml ? `<div class="memory-back-label">${backHtml}</div>` : ""}${specs.length ? `<div class="memory-spec-list">${specs.map(([label, value]) => `<div><b>${escapeHtml(label)}</b>${escapeHtml(value)}</div>`).join("")}</div>` : ""}</details>` : ""}
      <button id="recordKnowledgeBtn" class="memory-knowledge-link" type="button"><small>知識の蔵</small><strong>この一本を、もっと知る</strong><span>米や精米歩合、造り方など、この酒に関係するところから案内します。 →</span></button>
...
function openRecordKnowledge(r) {
  const items = [];
  const add = (title, value, body) => {
    if (value) items.push({ title, value, body });
  };
  add(
    "特定名称・酒類区分",
    r.classification,
    "名前は、原料や精米歩合、醸造方法などの違いを知る入口です。まずは難しい分類を覚えるより、今飲んだ一本の表示から少しずつ辿れます。",
  );
  add(
    "使用米・原料米",
    r.rice_variety,
    "酒米や原料米は、品種だけで味が決まるものではありません。蔵の造りや精米、酵母などと重なって、この一本の個性になります。",
  );
  add(
    "精米歩合",
    r.polishing_ratio,
    "精米歩合は、玄米を磨いたあとに残った割合です。数字が小さいほど多く磨いていますが、小さいほど必ずおいしい、という意味ではありません。",
  );
  add(
    "原材料",
    r.rice,
    "日本酒の基本は米・米こうじ・水。表示を見ると、この一本が何から造られているかを確認できます。",
  );
  add(
    "アルコール",
    r.alcohol,
    "アルコール度数も口当たりや飲みごたえを感じる手がかりのひとつです。温度や料理との組み合わせでも印象は変わります。",
  );
  const title =
    [r.brand_name, r.product_name].filter(Boolean).join(" ") || "この一本";
  $("detailBody").innerHTML = `
   ...
   `,
           ...
           .join("")
       : `<div class="msg info">この一本から案内できる知識を準備中です。</div>`
   }
...
function editDateValue(value) {
  const date = new Date(value || Date.now());
  if (Number.isNaN(date.getTime())) return todayDateValue();
  const offset = date.getTimezoneOffset() * 60000;
  return new Date(date.getTime() - offset).toISOString().slice(0, 10);
}
function editedDateISOString() {
  const value = $("eDrankDate")?.value;
  if (!value) return new Date().toISOString();
  const date = new Date(value + "T12:00:00");
  return Number.isNaN(date.getTime())
    ? new Date().toISOString()
...
function editPhotoMarkup(photoType, label, inputId, deleteId) {
  const photo = (S.detailPhotos || []).find((item) => item.photo_type === photoType);
  const url = photo?.storage_path
    ? photoObjectUrls.get(photo.storage_path)
    : "";
  return `<div class="memory-edit-photo">
    <div class="memory-edit-photo-preview">${url ? `<img src="${escapeHtml(url)}" alt="${escapeHtml(label)}" />` : "写真なし"}</div>
    <strong>${escapeHtml(label)}</strong>
    <div class="memory-edit-photo-actions">
      <label><input id="${inputId}" type="file" accept="image/*">${photo ? "変更" : "追加"}</label>
      ${photo ? `<label class="delete-photo"><input id="${deleteId}" type="checkbox"> 削除</label>` : `<input id="${deleteId}" class="hidden" type="checkbox">`}
    </div>
  </div>`;
}
function renderRecordEditor(r) {
  ...
  $("detailBody").innerHTML = `
   <div class="memory-edit">
     <div class="memory-edit-heading"><span>MY LOG</span><h2>記録を編集</h2></div>
     <section class="memory-edit-section">
       <h3>あの日のこと</h3>
       <div class="memory-edit-field"><label for="eDrankDate">日付</label><input id="eDrankDate" type="date" value="${escapeHtml(editDateValue(r.drank_at))}"></div>
       <div class="memory-edit-field"><label>評価</label><div class="edit-rating-choices">${[1, 2, 3, 4, 5].map((value) => `<button class="edit-rating-choice${Math.round(rating) === value ? " is-selected" : ""}" type="button" data-rating="${value}">${value}</button>`).join("")}</div><input id="eRating" type="number" min="1" max="5" step="0.5" value="${escapeHtml(rating)}"></div>
       <div class="memory-edit-field"><label for="eCompanion">誰と <small>任意</small></label><input id="eCompanion" value="${escapeHtml(r.companion_name || "")}"></div>
       <div class="memory-edit-field"><label for="eRestaurant">どこで <small>任意</small></label><input id="eRestaurant" value="${escapeHtml(r.restaurant_name || "")}"></div>
       <div class="memory-edit-field"><label for="eComment">ひとこと <small>任意</small></label><textarea id="eComment">${escapeHtml(r.comment || "")}</textarea></div>
     </section>
     <section class="memory-edit-section">
       <h3>写真</h3>
       <div class="memory-edit-photos">${editPhotoMarkup("front", "表ラベル", "editFrontPhoto", "deleteFrontPhoto")}${editPhotoMarkup("back", "裏ラベル", "editBackPhoto", "deleteBackPhoto")}${editPhotoMarkup("food", "料理の写真", "editFoodPhoto", "deleteFoodPhoto")}${editPhotoMarkup("memory", "思い出の写真", "editMemoryPhoto", "deleteMemoryPhoto")}</div>
     </section>
     <details class="memory-edit-details"><summary>銘柄情報を確認・修正</summary>
       <label>銘柄</label><input id="eBrand" value="${escapeHtml(r.brand_name || "")}">
       <label>商品名</label><input id="eProduct" value="${escapeHtml(r.product_name || "")}">
       <label>蔵元</label><input id="eBrewery" value="${escapeHtml(r.brewery_name || "")}">
       <label>都道府県</label><input id="ePrefecture" value="${escapeHtml(r.prefecture || "")}">
       <label>特定名称・酒類区分</label><input id="eClass" value="${escapeHtml(r.classification || "")}">
       <label>原材料</label><input id="eRice" value="${escapeHtml(r.rice || "")}">
       <label>使用米・原料米</label><input id="eRiceVariety" value="${escapeHtml(r.rice_variety || "")}">
       <label>精米歩合</label><input id="ePolishing" value="${escapeHtml(r.polishing_ratio || "")}">
       <label>アルコール</label><input id="eAlcohol" value="${escapeHtml(r.alcohol || "")}">
  ...
  document.querySelectorAll(".edit-rating-choice").forEach((button) => {
    button.onclick = () => {
      $("eRating").value = button.dataset.rating;
    ...
    try {
            ...
            if (old?.id) {
              try {
                ...
                try {
                  await deleteRows(
                    "sake_photos",
                    "id=eq." + encodeURIComponent(newRow.id),
                  );
                } catch {}
                try {
                  await deleteStorageObject(newPath);
                } catch {
                  queuePendingStorageDelete(newPath);
                }
                throw e;
              }
              ...
              try {
                await deleteStorageObject(old.storage_path);
              } catch {
                queuePendingStorageDelete(old.storage_path);
              }
            }
          } catch (e) {
      ...
      if (recordTextUpdated) {
        try {
          ...
          console.warn("record edit rollback failed", rollbackError);
        }
      }
      msg(
        $("editMsg"),
...
async function deleteCurrentRecord() {
  ...
  try {
    ...
    const item = {
      recordId: r.id,
      storagePaths: (photos || []).map((p) => p.storage_path).filter(Boolean),
    };
...
async function loadRecent() {
  ...
  try {
    const rows = await select(
      "drinking_records",
      "select=*&order=drank_at.desc&limit=6",
    );
...
async function loadHistory() {
  ...
  try {
    const rows = await select(
      "drinking_records",
      "select=*&order=drank_at.desc&limit=500",
    );
    S.historyRows = rows;
...
function historyMonthValue(value) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "";
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;
}

function populateHistoryMonths(rows) {
  const selectEl = $("historyYearMonth");
  if (!selectEl) return;
  const current = selectEl.value;
  const months = [...new Set(rows.map((r) => historyMonthValue(r.drank_at)).filter(Boolean))];
  selectEl.innerHTML = '<option value="">すべての年月</option>' + months.map((month) => {
    const [year, number] = month.split("-");
    return `<option value="${escapeHtml(month)}">${escapeHtml(year)}年 ${Number(number)}月</option>`;
  }).join("");
  if (months.includes(current)) selectEl.value = current;
}

function applyHistoryFilters() {
  const keyword = ($("historyKeyword")?.value || "").trim().toLocaleLowerCase("ja");
  const month = $("historyYearMonth")?.value || "";
  const minRating = Number($("historyMinRating")?.value || 0);
  const rows = S.historyRows.filter((r) => {
    const searchable = [r.brand_name, r.product_name, r.brewery_name, r.restaurant_name, r.companion_name, r.comment, r.prefecture]
      .filter(Boolean).join(" ").toLocaleLowerCase("ja");
    return (!keyword || searchable.includes(keyword)) &&
      (!month || historyMonthValue(r.drank_at) === month) &&
...
function renderRank(items, formatter = (v) => v) {
  if (!items.length)
    return '<div class="small">まだ集計できる記録がありません。</div>';
...
async function loadAnalysis() {
  ...
  try {
    ...
    const uniqueBreweries = new Set(
      rows.map((r) => r.brewery_name).filter(Boolean),
    ).size;
    const uniquePrefectures = new Set(
      rows.map((r) => r.prefecture).filter(Boolean),
    ).size;
    const uniqueBrands = new Set(
      rows
        .map((r) => [r.brand_name, r.product_name].filter(Boolean).join(" "))
        .filter(Boolean),
    ).size;
...
if ($("historyFilterToggle")) {
  $("historyFilterToggle").onclick = () => {
    ...
    const expanded = $("historyFilterToggle").getAttribute("aria-expanded") === "true";
    $("historyFilterToggle").setAttribute("aria-expanded", String(!expanded));
    panel.classList.toggle("hidden", expanded);
    if (!expanded) $("historyKeyword")?.focus();
  };
}
["historyKeyword", "historyYearMonth", "historyMinRating"].forEach((id) => {
